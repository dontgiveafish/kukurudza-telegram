...
custom_ip_address    = false # default "10.0.0.19" IP Address IPv4 private network range
...
project_host_alias = $custom_project_host_alias || "www.#{project_name}.dev"
ip_address = $custom_ip_address || "10.0.0.19"
default_host = $custom_default_host || "localserver"
hostmanager_aliases = ["#{project_host}", "#{project_host_alias}"]

# Environment variables

ARCH = $custom_vm_arch || ENV['VM_ARCH'] || 64
...
$virtual_host = <<SCRIPT
    ...
    echo "<VirtualHost *:80>
      ...
      LogLevel info
      ErrorLog /var/www/log/#{project_name}/apache-error.log
      CustomLog /var/www/log/#{project_name}/apache-access.log combined
...
$script = <<SCRIPT
    ...
    # Setup project dir
    mkdir -p /var/www/log/#{project_name}
    chown www-data:www-data -R /var/www/log/#{project_name}

    # Install PHP
    apt-get install -qq -y php5 php5-cli php5-common php5-gd php5-mysql php5-curl php5-intl php5-mcrypt php5-tidy php5-readline php5-xdebug php-apc php5-memcached php5-memcache php5-imap php5-sqlite php5-xsl php-pear
    ...
    # Add database
    mysql -u root -plocal -e "CREATE DATABASE #{project_name}  CHARACTER SET utf8 COLLATE utf8_general_ci;"
